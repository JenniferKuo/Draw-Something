...
io.sockets.on('connection',function (socket) {
    socket.on('login',function (name) {
        this.name = name || "我沒有名字";
        this.attrin = false;
        this.emit('server msg','歡迎, '+this.name+' !');
        this.broadcast.emit('server msg','歡迎, '+this.name+' !');
        ...
        this.on('in',function () {
            ...
            setTimeout(function () {
                ...
                Game.player = t;
                t.time = 40;t.word = db.randomWord();
                ...
                Game.timer = setTimeout(function () {
                    ...
                    t.time--;var o = {name:t.name,time:t.time,word:t.word.word.length+'個字'};
                    if(t.time <= 20) {
        ...
        this.on('client msg',function (msg) {
            if(!doCmd(msg,this)) {
                ...
                if(Game.player && Game.player.word.word === msg){
                    ...
                    this.emit('win msg',"你很BON！答對惹！答案是"+msg);
                    this.broadcast.emit('win msg',"恭喜！"+this.name+" 答對啦！");
                    var j = JSON.stringify(tops);
                    this.broadcast.emit('tops',j);
                    this.emit('tops',j);
                    this.prev = {
                ...
                // this.emit('server msg', this.name  + ' 說:   ' + msg);
                // this.broadcast.emit('server msg', this.name  + ' 說:   ' + msg);
                var chat_name = this.name;
                this.emit('chat name',chat_name);
                this.broadcast.emit('chat name',chat_name);
                this.emit('chat msg',msg);
                this.broadcast.emit('chat msg',msg);
            }
        });
        this.on('disconnect',function () {
            ...
            this.broadcast.emit('server msg','888888, '+this.name +'。');
            this.broadcast.emit('out user',this.id.substring(2));
