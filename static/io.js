...
socket.on('chat msg',function(data){
    ...
    bubble.id='';
    bubble.className = 'talk-bubble tri-right left-in';
    msg.appendChild(bubble);

    var content = document.createElement('div');
    content.className = 'talktext';
    bubble.appendChild(content);
    content.innerHTML = data;
})
socket.on('win msg',function(data){
    var content = document.getElementById('element_to_pop_up');
    content.innerHTML = data;
...
socket.on('login',function () {
    if(prompt)
        socket.emit('login',prompt('請輸入名字'));
    else
        socket.emit('login','手機用戶');
    btnIn.outAct();
    canvas.isMe = false;
...
socket.on('timeout',function (d) {
    ...
    if(t) t.outerHTML='';
    info.time.innerText = '時間到啦~';
    info.word.innerText = '正確答案是：'+d.word;
});
